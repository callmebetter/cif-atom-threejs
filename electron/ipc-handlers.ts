...
export function setupIpcHandlers(): void {
  ...
  ipcMain.handle(
    "save-file",
    async (_, [fileName, data]: [string, ArrayBuffer]) => {
  ...
  ipcMain.handle(
    ...
    async (_, projectId: number, analysisType?: number) => {
      try {
  ...
  // Old CIF record handlers (deprecated - use new cif: handlers)
  ...
  // New CIF handlers using normalized schema
  ipcMain.handle(
    "cif:save",
    async (_, filePath: string, cifContent?: string) => {
      try {
        const recordId = await db.cifService.saveCifFile(filePath, cifContent);
        return { success: true, recordId };
      } catch (error) {
        return { success: false, error: (error as Error).message };
      }
  ...
  ipcMain.handle("cif:get-complete", async (_, id: number) => {
    try {
      const data = await db.cifService.getCifData(id);
  ...
  ipcMain.handle("cif:query", async (_, options?: any) => {
    try {
      const results = await db.cifService.queryCifRecords(options);
  ...
  ipcMain.handle("cif:delete", async (_, id: number) => {
    try {
      const success = await db.cifService.deleteCifRecord(id);
      return { success };
    } catch (error) {
      return { success: false, error: (error as Error).message };
    }
  ...
  ipcMain.handle("cif:get-summary", async (_, id: number) => {
    try {
      const cifData = await db.cifService.getCifData(id);
      if (cifData) {
        const summary = db.cifService.generateStructureSummary({
          info: cifData.info,
          lattice: cifData.lattice,
          atoms: cifData.atoms,
          elements: cifData.elements,
        });
        return { success: true, summary };
      }
      return { success: false, error: "CIF record not found" };
    } catch (error) {
      return { success: false, error: (error as Error).message };
    }
  });
