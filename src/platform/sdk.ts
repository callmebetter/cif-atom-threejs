...
export const fileOperations = {
  ...
  saveFile: (fileName: string, data: ArrayBuffer) =>
    safeInvoke("save-file", [fileName, data]),
...
export const databaseOperations = {
  // Project operations
  createProject: (
    project: Omit<ProjectRecord, "id" | "created_at" | "updated_at">,
  ...
  createAnalysisRecord: (
    analysis: Omit<AnalysisRecord, "id" | "created_at" | "completed_at">,
  ...
  deleteAnalysisRecord: (id: number) =>
    safeInvoke("db:delete-analysis-record", id),

  // Settings operations
  getSetting: (key: string) => safeInvoke("db:get-setting", key),
  setSetting: (key: string, value: string) =>
    safeInvoke("db:set-setting", [key, value]),
  ...
  updateCifRecord: (id: number, updates: any) =>
    safeInvoke("db:update-cif-record", [id, updates]),
  ...
  cif: {
    save: (filePath: string, cifContent?: string) =>
      safeInvoke("cif:save", [filePath, cifContent]),
    getComplete: (id: number) => safeInvoke("cif:get-complete", id),
    query: (options?: any) => safeInvoke("cif:query", options),
    delete: (id: number) => safeInvoke("cif:delete", id),
    getSummary: (id: number) => safeInvoke("cif:get-summary", id),
  },
};
