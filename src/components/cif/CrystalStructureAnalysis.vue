<template>
    <div class="analysis-section">
        ...
        <el-descriptions :column="2" border>
            <el-descriptions-item label="晶胞体积"
                >{{
                    analysisResult.data.cellVolume.toFixed(3)
                }}
                Å³</el-descriptions-item
            >
            <el-descriptions-item label="密度"
                >{{
                    analysisResult.data.density.toFixed(3)
                }}
                g/cm³</el-descriptions-item
            >
            <el-descriptions-item label="化学式">{{
                analysisResult.data.formula
            }}</el-descriptions-item>
            <el-descriptions-item label="晶系">{{
                analysisResult.data.symmetry.crystalSystem
            }}</el-descriptions-item>
            <el-descriptions-item label="空间群">{{
                analysisResult.data.symmetry.spaceGroup
            }}</el-descriptions-item>
            <el-descriptions-item label="点群">{{
                analysisResult.data.symmetry.pointGroup
            }}</el-descriptions-item>
        </el-descriptions>
        ...
        <el-table :data="coordinationData" style="width: 100%" max-height="200">
            <el-table-column prop="element" label="元素" width="100">
                <template #default="scope">
                    <el-tag
                        :color="getElementColor(scope.row.element)"
                        effect="dark"
                    >
                        {{ scope.row.element }}
                    </el-tag>
        ...
        <h3>键长分析</h3>
        <el-table
            :data="analysisResult.data.bondLengths"
            style="width: 100%"
            max-height="200"
        >
        ...
        <h3>键角分析</h3>
        <el-table
            :data="analysisResult.data.angles"
            style="width: 100%"
            max-height="200"
        >
            <el-table-column prop="element1" label="原子1" width="80" />
...
import { CifParser as CifParserV1 } from "@/utils/cifParserV1";
...
const coordinationData = computed(() => {
    ...
    console.log(
        `props.analysisResult.data.composition, `,
        props.analysisResult,
    );
    return Array.from(props.analysisResult.data.composition.entries()).map(
        ([element]) => ({
            element,
...
const getElementColor = (element: string) => {
    return CifParserV1.getElementColor(element);
};
</script>
