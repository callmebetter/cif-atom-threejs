...
const compositionData = computed(() => {
    ...
    const totalAtoms = Array.from(composition.values()).reduce(
        (sum, count) => sum + count,
        0,
    );
    ...
    return Array.from(composition.entries()).map(([element, count]) => {
        const atomicWeight =
            props.materialAnalyzer["atomicWeights"].get(element) || 0;
        const massContribution =
            ((count * atomicWeight) /
                Array.from(composition.entries()).reduce(
                    (sum, [elem, cnt]) =>
                        sum +
                        cnt *
                            (props.materialAnalyzer["atomicWeights"].get(
                                elem,
                            ) || 0),
                    0,
                )) *
